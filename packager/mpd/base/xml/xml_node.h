...
class XmlNode {
 public:
  ...
  [[nodiscard]] bool SetFloatingPointAttribute(const std::string& attribute_name,
                                 double number);
...
class RepresentationXmlNode : public RepresentationBaseXmlNode {
 public:
  ...
  [[nodiscard]] bool AddLiveOnlyInfo(const MediaInfo& media_info,
 ...
 private:
  ...
  [[nodiscard]] bool AddAudioChannelInfo(const MediaInfo::AudioInfo& audio_info);

  // Add audioSamplingRate attribute to this element, if present.
  [[nodiscard]] bool AddAudioSamplingRateInfo(const MediaInfo::AudioInfo& audio_info);
